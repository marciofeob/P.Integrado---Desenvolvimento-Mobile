...
/// - `ItemCode;Quantidade` (depósito usa o padrão)
...
class _ImportPageState extends State<ImportPage> {
  ...
  List<_ItemImportado> _parseCsv(String conteudo) {
    ...
    // Detecta delimitador: se a primeira linha tem mais `;` que `,`, usa `;`
    ...
    if (temHeader) {
      ...
      for (int i = 0; i < colunas.length; i++) {
        ...
        if (col.contains('item') || col.contains('código') ||
            col.contains('codigo') || col.contains('code')) {
          colItem = i;
        } else if (col.contains('quantidade') || col.contains('qty') ||
            col.contains('quantity') || col.contains('contad')) {
          colQtd = i;
        } else if (col.contains('depósito') || col.contains('deposito') ||
            col.contains('warehouse') || col.contains('wh')) {
          colDeposito = i;
        }
      }
    }
    ...
    for (int i = inicio; i < linhas.length; i++) {
      ...
      // Quantidade: tenta a coluna mapeada, senão coluna 1
      ...
      // Depósito: tenta a coluna mapeada, senão '01'
      String deposito = '01';
      if (colDeposito < campos.length && colDeposito != colItem &&
          colDeposito != colQtd) {
  ...
  Future<void> _importarContagens() async {
    ...
    final confirmar = await StoxDialog.confirmar(
      ...
      mensagem:
          'As contagens serão adicionadas ao banco local e ficarão pendentes para sincronização.\n\nDeseja continuar?',
    ...
    try {
      ...
      StoxSnackbar.sucesso(
        context,
        '$importados ${importados == 1 ? 'item importado' : 'itens importados'} com sucesso!',
      );

      // Volta ao painel para sincronizar
                  ...
                  children: [
                    ...
                    if (_nomeArquivo != null && _itens.isNotEmpty) ...[
                      ...
                      if (!_importado)
                        StoxButton(
                          label:
                              'IMPORTAR ${_itens.length} ${_itens.length == 1 ? 'ITEM' : 'ITENS'}',
                          icon: Icons.download_rounded,
                          onPressed: _importarContagens,
                        ),
                      if (_importado) _buildSucessoImportacao(theme),
                    ],
                    if (_itens.isEmpty && _nomeArquivo == null && _erro == null)
  ...
  Widget _buildBotaoSelecionar(ThemeData theme) {
    return StoxButton(
      label: _itens.isEmpty ? 'SELECIONAR ARQUIVO CSV' : 'TROCAR ARQUIVO',
  ...
  Widget _buildListaItens(ThemeData theme) {
    return Column(
      ...
      children: [
        Row(children: [
          Icon(Icons.list_alt_rounded, color: Colors.grey.shade600, size: 18),
          const SizedBox(width: 8),
          Text(
            'Preview dos itens',
            style: TextStyle(
  ...
  Widget _buildSucessoImportacao(ThemeData theme) {
    return Container(
